			...
			<div class="row">
				...
				<div class="col-md-8">
					...
					<c:if
						...
						<a href="../laptop" class="btn btn-info">Continue Shopping</a>
					</c:if>
					<c:if
						test="${!empty MY_CART_ITEMS && !MY_CART_ITEMS.orderItems.isEmpty()}">
							...
							<tbody>
								...
								<c:forEach items="${MY_CART_ITEMS.orderItems}" var="item"
									...
									<tr>
										...
										<td>${item.quantity}</td>
										<td>Rs.${item.laptop.price*item.quantity }</td>
										<td><a href="../lorderItems/lremove?id=${loop.index}"
					...
					<div class="col-md-4">
						...
						<c:forEach items="${MY_CART_ITEMS.orderItems}" var="item">
							<c:set var="total_amount"
								value="${total_amount + item.laptop.price*item.quantity}" />
						</c:forEach>

						<form name="orderForm" action="../lorders/lsave" method="post">
